"""Unit tests of the subscribe society view for a student account"""
...
class FollowSocietyViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('subscribe_society')
    
    def test_url(self):
        self.assertEqual(self.url, '/subscribe_society/')
        
    def test_successful_subscribe_society(self):
        self.client.login(username=self.user.email, password='Password123')
        before_count = self.society.subscriber.count()
        ...
        after_count = self.society.subscriber.count()
        self.assertEqual(before_count+1, after_count)
        
    def test_successful_unsubscribe_society(self):
        self.society.add_subscriber(self.user)
        self.client.login(username=self.user.email, password='Password123')
        before_count = self.society.subscriber.count()
        response = self.client.post(self.url, {'society_pk': self.society.pk}, follow=True)
        response_url = reverse('society_page',  kwargs={'pk': self.society.pk})
